...
function multi_average(array,num){
	for(var i = 0; i<array.length; i+=num){
		for(var j = 0; j<array[i].length; j+=num){
			var average = 0;
			var count = 0;
			for(var innerI = i; innerI < i+num; innerI++){
				for(var innerJ = j; innerJ < j+num; innerJ++){
					if(typeof array[innerI] !== 'undefined' && !isNaN(array[innerI][innerJ])){
						average += array[innerI][innerJ];
						count++;
					}
				}
			}
...
console.log(multi_average(input_array,average_size));